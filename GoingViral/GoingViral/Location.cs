...
namespace GoingViral
{
	...
	public class Location
	{
		...
		public Location( string rawLocationInformation, int randomNumberSeed )
		{
			...
			NumberGenerator = new Random( randomNumberSeed );
		}
		...
		public void InfectNewHost( Virus theVirus )
		{
			if( Population > 0 )
			{
				if( InfectedHosts.Count > 0 && InfectedHosts.Count < 80 && InfectedHosts[ 0 ].NumberOfPeopleRepresentedByThisHost > 1 )
				{
					//We actually need to expand this array. This makes it so that the last 100000 people won't all die at once.
					double initialhostnumber = InfectedHosts.Count;
					for( int x = 0; x < initialhostnumber; ++x )
					{
						InfectedHosts[ x ].NumberOfPeopleRepresentedByThisHost /= 10;
						for( int y = 0; y < 10; y++ )
						{
							double daysInfected = InfectedHosts[ x ].DaysInfected;
							daysInfected += NumberGenerator.Next( ( int )( theVirus.IncubationPeriod_Days + theVirus.TimeToKill ) ) - ( ( theVirus.IncubationPeriod_Days + theVirus.TimeToKill ) / 2 );
							InfectedHosts.Add( new Host( theVirus, InfectedHosts[ x ].NumberOfPeopleRepresentedByThisHost, NumberGenerator.Next(), daysInfected ) );
						}
					}
				}
				if( InfectedHosts.Count == 0 || InfectedHosts.Count * InfectedHosts[ 0 ].NumberOfPeopleRepresentedByThisHost < Population )
				{
					//We need to compress this array. We don't have the processing power to handle more than 1000 hosts.
					if( InfectedHosts.Count >= 1000 )
					{
						for( int x = 0; x < 100; ++x )
						{
							InfectedHosts[ x ].NumberOfPeopleRepresentedByThisHost *= 10;
							double averagedaysinfected = 0;
							int numberofhoststoshrink = 0;
							for( int y = x * 10; y < ( x * 10 ) + 10; y++ )
							{
								averagedaysinfected += InfectedHosts[ y ].DaysInfected;
								numberofhoststoshrink++;
							}
							InfectedHosts[ x ].DaysInfected = averagedaysinfected / numberofhoststoshrink;
						}
					...
					{
						InfectedHosts.Add( new Host( theVirus, InfectedHosts[ 0 ].NumberOfPeopleRepresentedByThisHost, NumberGenerator.Next() ) );
					}
					else
					{
						InfectedHosts.Add( new Host( theVirus, 1.0, NumberGenerator.Next() ) );
					}
				}
			}
		}
